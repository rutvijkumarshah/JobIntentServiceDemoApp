 ...
 * complete}
 */
public abstract class JobIntentServiceWitFix extends Service {
    ...
    /**
     * Call this to enqueue work for your subclass of {@link JobIntentServiceWitFix}.  This will
    ...
    void processorFinished() {
        //Proposed-Fix : The async task has finished.
        mCurProcessor = null;
        if (mCompatQueue != null) {
            synchronized (mCompatQueue) {
                //mCurProcessor = null; Proposed-Fix AsyncTask(O and Post O) and mCompatQueue (Pre O) are mutual exclusive
    ...
    static final class JobServiceEngineImpl extends JobServiceEngine
            implements JobIntentServiceWitFix.CompatJobEngine {
        static final String TAG = "JobServiceEngineImpl";

        static final boolean DEBUG = false;

        final JobIntentServiceWitFix mService;
        final Object mLock = new Object();
        JobParameters mParams;

        JobServiceEngineImpl(JobIntentServiceWitFix service) {
            super(service);
            mService = service;
        }
         ...
         */
        @Override
        public JobIntentServiceWitFix.GenericWorkItem dequeueWork() {
            JobWorkItem work;
            synchronized (mLock) {
                if (mParams == null) {
                    return null;
        ...
        final class WrapperWorkItem implements JobIntentServiceWitFix.GenericWorkItem {
    ...
    @RequiresApi(26)
    static final class JobWorkEnqueuer extends JobIntentServiceWitFix.WorkEnqueuer {
